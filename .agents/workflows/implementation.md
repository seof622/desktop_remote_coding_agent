...
- 복잡한 소스코드 작업 또는 구현 관계를 정의·변경하는 Plan 문서 작업 완료 시에만
  `.agents/templates/completion-summary.md` 기준의 시각 요약을 제공한다. 변경·검증·Git 상태를 확인한
  사실만 사용하고 민감정보는 제외한다. 단순 문서·설정·Git 작업에는 생성하지 않는다.
